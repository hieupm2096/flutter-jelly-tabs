...
- **Review**: Stress-tested design/architecture/plan before starting S1. Resolved:
  - Pinned the RN source to commit `67f47f2` (was citing the `main` branch); confirmed MIT license
    is compatible with a from-scratch port; added a step to vendor a read-only snapshot at
  ...
  - Added explicit non-goal: no RTL/bidi mirroring (verified RN source has no RTL handling either).
  - Added keyboard/focus activation (Tab + Enter/Space) as new v0.1.0 scope for web/desktop — no
    RN equivalent to port, but needed given web is a stated target platform.
...
- **Review**: Fetched `use-distortion.ts` from the pinned commit (`67f47f2`) to resolve the open
  ...
  spring-driven in RN, and only as fire-and-forget "retarget once" springs — never continuously
  retargeted mid-drag. `translateY`/`scaleX`/`transformOriginX` during an active drag (`update()`)
  are plain synchronous assignments in RN, not springs. Conclusion: Flutter's `SpringSimulation`
